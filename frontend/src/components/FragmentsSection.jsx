  ...
  return (
    <section id="fragmenten" className={`py-16 md:py-20 ${background ? 'bg-[#f0f4fa]' : ''}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        {showHeader && <h2 className="text-[#062a4a] text-3xl md:text-4xl font-black mb-10">Palette</h2>}
          ...
          {items.map((item) => (
            <Link key={item.id} to={`/fragmenten/${item.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-[#d8e4f0]">
              <div className="relative aspect-[2/1] overflow-hidden bg-[#e4ecf5]">
                ...
                {item.audio && (
                  <div className="absolute top-3 left-3 inline-flex items-center gap-1.5 bg-white/95 backdrop-blur px-2.5 py-1.5 rounded-full text-[11px] font-bold text-[#062a4a] shadow">
                    <Headphones size={12} className="text-[#2a5d99]" /> Audio
                  </div>
                )}
              ...
              <div className="p-6">
                <h3 className="text-[#062a4a] text-lg font-bold leading-snug hover-pulse line-clamp-2 min-h-[3.5rem]">
                  {item.title}
                </h3>
              </div>
            </Link>
