...
const FragmentDetailPage = () => {
  ...
  const related = fragments.filter((n) => n.id !== item.id).slice(0, 3);
    ...
    <>
      <section className="relative overflow-hidden pt-28 md:pt-32" style={{ background: 'linear-gradient(180deg,#062a4a 0%,#0a3a6b 60%,#1f5499 100%)' }}>
        ...
        <div className="relative max-w-4xl mx-auto px-6 lg:px-10">
          <Link to="/fragmenten" className="inline-flex items-center gap-2 text-white/85 hover:text-white mb-6 font-medium text-sm">
      ...
      <article className="bg-white page-pad-bottom">
        <div className="max-w-3xl mx-auto px-6 lg:px-8 pt-10 md:pt-14">
          {item.audio && (
            <div className="mb-8">
              <CustomAudioPlayer src={item.audio} title={item.title} totalSeconds={item.audioDuration} />
            </div>
          )}
          <div className="space-y-5 text-[#2a3a4a] text-lg leading-relaxed">
        ...
        {related.length > 0 && (
          <div className="bg-[#f0f4fa] py-12 md:py-16 mt-12 md:mt-16">
            <div className="max-w-7xl mx-auto px-6 lg:px-10">
              <h2 className="text-[#062a4a] text-2xl md:text-3xl font-black mb-8">Meer Palette</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {related.map((b) => (
                  <Link key={b.id} to={`/fragmenten/${b.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-[#d8e4f0]">
                    ...
                    <div className="p-5">
                      <h3 className="text-[#062a4a] text-lg font-bold leading-snug hover-pulse line-clamp-2 min-h-[3.25rem]">{b.title}</h3>
                    </div>
                  </Link>
                ))}
              </div>
