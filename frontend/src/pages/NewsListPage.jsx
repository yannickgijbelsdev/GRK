...
const NewsListPage = () => {
  return (
    <>
      <PageHeader title="Nieuws uit de buurt" subtitle="Het laatste nieuws uit de wereld van R&B en hiphop." />
            ...
            {newsItems.map((item) => (
              <Link key={item.id} to={`/nieuws/${item.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-[#d8e4f0] flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <h3 className="text-[#062a4a] text-xl font-bold leading-snug mb-3 group-hover:underline decoration-[#2a5d99] decoration-2 underline-offset-4">{item.title}</h3>
